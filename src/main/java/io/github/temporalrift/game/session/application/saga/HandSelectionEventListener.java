...
/** Opens the declaration window only after every player has a terminal five-card hand. */
@Component
    ...
    void onHandSelected(HandSelected event) {
        eraSagaRepository
                ...
                .map(state -> {
                    ...
                    return selected.allHandsSelected()
                            ? selected.withStatus(EraSagaStatus.WAITING_DECLARATION)
                            : selected;
                })
                .ifPresent(selected -> {
                    ...
                    eventPublisher.publish(DomainEventEnvelope.create(
                            ...
                            event,
                            clock));
                    if (selected.status() == EraSagaStatus.WAITING_DECLARATION) {
                        applicationEventPublisher.publishEvent(
                                new HandSelectionCompleted(event.gameId(), event.eraNumber(), selected.playerIds()));
                    }
                });
    }
}
