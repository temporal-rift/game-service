...
class GetGameStateQueryHandler implements GetGameStateUseCase {
    ...
    public Result handle(Query query) {
        ...
        requireParticipant(lobby, query);
        return new Result(
                game.id(),
                game.status(),
                game.eraCounter(),
    ...
    private void requireParticipant(Lobby lobby, Query query) {
        // Same 404 as an unknown game so outsiders cannot probe which gameIds exist.
        lobby.currentPlayers().stream()
                .filter(player -> player.playerId().equals(query.callerPlayerId()))
                .findAny()
                .orElseThrow(() -> new GameNotFoundException(query.gameId()));
    }
}
