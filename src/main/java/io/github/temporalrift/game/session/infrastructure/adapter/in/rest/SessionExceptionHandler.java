...
import io.github.temporalrift.game.session.domain.lobby.InvalidPlayerNameException;
...
class SessionExceptionHandler {
    ...
    ProblemDetail handlePlayerNotInLobby(PlayerNotInLobbyException ex) {
        return ProblemDetails.of(HttpStatus.NOT_FOUND, ex.getMessage(), "404-03");
    }
    ...
    ProblemDetail handleDisconnectedPlayers(DisconnectedPlayersException ex) {
        var problemDetail = ProblemDetails.of(HttpStatus.CONFLICT, ex.getMessage(), "409-03");
    ...
    ProblemDetail handleNotHost(NotLobbyHostException ex) {
        return ProblemDetails.of(HttpStatus.FORBIDDEN, ex.getMessage(), "403-02");
    }
